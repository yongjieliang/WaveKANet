...
import math
from network.kan import KANLinear
...
class DWT_2D(nn.Module):
    ...
    def get_matrix(self):
        ...
        self.matrix_low_0 = torch.tensor(matrix_h_0, dtype=torch.float32).cuda()
        self.matrix_low_1 = torch.tensor(matrix_h_1, dtype=torch.float32).cuda()
        self.matrix_high_0 = torch.tensor(matrix_g_0, dtype=torch.float32).cuda()
        self.matrix_high_1 = torch.tensor(matrix_g_1, dtype=torch.float32).cuda()
...
class IntelligentWaveletPoolingModule(nn.Module):
    def __init__(self, in_channels, wavename='haar'):
        ...
        self.high_freq_conv = nn.Conv2d(self.in_channels * 3, self.in_channels * 3, kernel_size=1)  # 保持通道不变
        self.softmax = nn.Softmax(dim=1)  # 注意：Softmax2d 已弃用，使用 dim=1 是对通道归一化
        self.fusion_conv = nn.Conv2d(self.in_channels + self.in_channels, self.in_channels, kernel_size=1)
    ...
    def forward(self, input):
        ...
        C = self.in_channels
        enhanced_chunks = torch.chunk(enhanced_high_freq, 3, dim=1)  # List of (B, C, H/2, W/2)
        high_freq_avg = sum(enhanced_chunks) / 3  # (B, C, H/2, W/2)

        combined_features = torch.cat([LL, high_freq_avg], dim=1)  # (B, 2C, H/2, W/2)
        output = self.fusion_conv(combined_features)  # (B, C, H/2, W/2)
        return output
...
class GroupKANChannelBlock(nn.Module):
    def __init__(self, in_channels, group=3, grid_size=5, spline_order=3):
...
# ============================================================
class MultiScaleGroupKANTransform(nn.Module):
    """
...
class WaveKANet(nn.Module):
    def __init__(self,
        ...
        # MultiScaleGroupKANTransform
        self.msgkt1 = MultiScaleGroupKANTransform(dim=dim, num_gkt_layers=num_gkt_layers, group_kan_num=group_kan_num, drop=drop, local_expansion=local_expansion)
        self.msgkt2 = MultiScaleGroupKANTransform(dim=dim, num_gkt_layers=num_gkt_layers, group_kan_num=group_kan_num, drop=drop, local_expansion=local_expansion)
        self.msgkt3 = MultiScaleGroupKANTransform(dim=dim, num_gkt_layers=num_gkt_layers, group_kan_num=group_kan_num, drop=drop, local_expansion=local_expansion)
        self.msgkt4 = MultiScaleGroupKANTransform(dim=dim, num_gkt_layers=num_gkt_layers, group_kan_num=group_kan_num, drop=drop, local_expansion=local_expansion)

        # IntelligentWaveletPooling
        self.iwp1 = IntelligentWaveletPoolingModule(in_channels=dim)
        self.iwp2 = IntelligentWaveletPoolingModule(in_channels=dim)
        self.iwp3 = IntelligentWaveletPoolingModule(in_channels=dim)
        self.iwp4 = IntelligentWaveletPoolingModule(in_channels=dim)

        # classifier
        self.classifier = nn.Linear(dim * (height // 16) * (width // 16) , num_classes)
...
def wavekanet():
    return WaveKANet(dim=3, height=256, width=256, num_classes=2, num_gkt_layers=3, group_kan_num=3, drop=0.0, local_expansion=1)
